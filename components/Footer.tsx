    ...
    <footer id="contacto" className="bg-gray-900 text-white py-12">
      <div className="container mx-auto px-4">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {/* Info de Contacto */}
          <div>
            <h3 className="text-xl font-bold mb-4">{tenant.nombre}</h3>
            <p className="text-gray-400 mb-4">
              {footer.descripcion}
            </p>
            <div className="space-y-2 text-gray-400">
              {footer.email && <p>📧 {footer.email}</p>}
              {footer.telefono && <p>📱 {footer.telefono}</p>}
              {footer.direccion && <p>📍 {footer.direccion}</p>}
            </div>
          </div>
          ...
          <div>
            ...
            <div className="flex gap-4">
              {footer.redes_sociales.facebook && (
                <a
                  href={footer.redes_sociales.facebook}
                  target="_blank"
              ...
              {footer.redes_sociales.instagram && (
                <a
                  href={footer.redes_sociales.instagram}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="w-10 h-10 bg-gray-800 hover:bg-primary rounded-full flex items-center justify-center transition"
                >
              ...
              {footer.redes_sociales.whatsapp && (
                <a
                  href={`https://wa.me/${footer.redes_sociales.whatsapp.replace(/[^0-9]/g, '')}`}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="w-10 h-10 bg-gray-800 hover:bg-primary rounded-full flex items-center justify-center transition"
                >
